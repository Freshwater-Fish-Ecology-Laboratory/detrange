...
fit <- dr_fit(data)
```
Check model convergence with `dr_glance()`. Try adjusting the `nthin` argument in `dr_fit()` to improve convergence. Higher `nthin` generates more sample iterations.
